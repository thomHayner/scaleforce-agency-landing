...
The source repo `thomHayner/thomHayner.com` is now private. The loader's unauthenticated GitHub API calls returned 404 and silently yielded zero syndicated posts. Fix: the loader now reads an optional `TH_BLOG_TOKEN` (fine-grained PAT with `contents:read` on the source repo) and sends it as `Authorization: Bearer` on every GitHub request. Absent token → current graceful-empty behavior preserved, so CI builds without the secret (e.g. forks, GitHub Actions with its own scoped `GITHUB_TOKEN`) still pass. Production/preview Vercel environments need the secret set to render syndicated posts.
